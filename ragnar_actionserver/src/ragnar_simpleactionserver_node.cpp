                         ...
                         ros::Publisher& trajectory_pub)
{
  ...
  trajectorypoint.pose.position.x = mobile_command[0];
  trajectorypoint.pose.position.y = mobile_command[1];
  trajectorypoint.pose.position.z = mobile_command[2];
  trajectorypoint.twist.linear.x = mobile_command[3];
  trajectorypoint.twist.linear.y = mobile_command[4];
  trajectorypoint.twist.linear.z = mobile_command[5];
  trajectorypoint.accel.linear.x = mobile_command[6];
...
int main(int argc, char** argv)
{
  ...
  ros::Publisher trajectory_command_pub = nh.advertise<ragnar_msgs::CartesianTrajectoryPoint>("mobile_platform_trajectory", 1);
  ...
  ros::Timer state_publish_timer =
      nh.createTimer(ros::Duration(1.0/publish_rate), boost::bind(publishCurrentCommand,
                                                     ...
                                                     boost::ref(trajectory_command_pub)));
